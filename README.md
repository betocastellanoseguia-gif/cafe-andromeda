...
### Input validation
- Dedicated capture functions that raise `ValueError` with descriptive messages instead of failing silently
- Menu options validated against a whitelist tuple before dispatch

### Error handling
- Specific handling for `ValueError`, `FileNotFoundError`, `PermissionError`, `json.JSONDecodeError` and `KeyboardInterrupt`
...
### Persistence
- JSON output with UTF-8 encoding and indentation, including session metadata
